...
6. If `git push` fails because the local branch is behind remote and `git fetch` is blocked, use the GitHub plugin's `push_files` tool to push file changes directly via the API instead of git.

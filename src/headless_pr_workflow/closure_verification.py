...
def parse_issue_number(argument_text: str) -> int:
    ...
    if len(tokens) != 1 or not tokens[0].isdigit() or tokens[0].startswith("0"):
        raise VerificationBlocked("exactly one positive integer issue number is required")
    number = int(tokens[0])
    if number <= 0:
        raise VerificationBlocked("exactly one positive integer issue number is required")
    return number
...
def _inventory(github: GitHubEvidence, target: Issue) -> tuple[InventoryItem, ...]:
    ...
    metadata = {child.number: child for child in github.metadata_children(target.number)}
...
def verify_closure(
    ...
    if target_type == "type:epic":
        ...
        if open_features:
            ...
            raise VerificationBlocked(f"Epic has open or invalid child Features: {joined}")

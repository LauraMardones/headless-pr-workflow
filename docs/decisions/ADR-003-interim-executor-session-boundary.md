...
**Correction 1 (post-review, same day):** the first version of this ADR claimed the four per-turn caps above kept every run "beneath" GitHub Actions' 6-hour job ceiling. That was wrong, and review caught it before merge: `AGENT_MAX_TURNS × (AGENT_API_TIMEOUT + AGENT_TOOL_TIMEOUT)` = 60 × 900s = 54,000s ≈ **15 hours** — two and a half times the ceiling, not beneath it. Worse, relying on GitHub Actions to force-kill an over-budget job is not an acceptable backstop on its own: an abrupt platform kill can happen mid-step and bypass the loop's own non-zero return, which is what the caller depends on to post a `## Blocked Declaration` — silent failure instead of a reported one. `AGENT_MAX_WALLCLOCK_SECONDS` was added to fix this, checked inside the loop against elapsed wall-clock time.

**Correction 2 (post-review, same day):** the fix in Correction 1 was still wrong in scope. It measured elapsed time from when *that particular `invoke_executor_command()` call* started — but one story calls `invoke_executor_command()` up to **four separate times** (`/implement`, `/review`, `/merge`, `/cleanup`), and the dispatcher's `exec bash "$0"` tail-call to the next ready story replaces the process without starting a new Actions job. All of that — every phase of every story processed in one triggering event — shares a single 6-hour ceiling. A budget that resets on each of those four calls (and again on every `exec`) does not bound the job at all: one story alone could consume up to ~4 × 3h, before even considering a second story in the same run.
...
- `AGENT_MAX_WALLCLOCK_SECONDS`, checked against `DISPATCH_JOB_START_TS`, bounds the worst case directly at the *job* level: no combination of phases (`/implement`/`/review`/`/merge`/`/cleanup`) or chained stories within one Actions job can approach the platform's 6h ceiling, regardless of turn count, and the script always fails closed through its own exit path rather than relying on a platform-level kill.
